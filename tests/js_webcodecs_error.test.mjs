...
test('happy path: one chunk per frame', async () => {
    MODE='immediate';
    ...
    assert.ok(!thrown, `immediate encoder: push resolves (${thrown?.message ?? ''})`);
    assert.ok(c?.key===true && c?.data?.length===3, 'immediate encoder: chunk shape');
    assert.ok((await enc.close()).length===0, 'immediate encoder: nothing left at close');

});

test('lag > 0: the encoder holds frames back; push() must still resolve', async () => {
    MODE='lag';
    const enc=createTrackEncoder({ kind:'luma', lossless:true, W, H, fps:FPS });
    let c=null, thrown=null;
    try{ c=await withTimeout(enc.push(plane()), 2000, 'lagging push'); }catch(e){ thrown=e; }
    assert.ok(!thrown, `lagging encoder: push resolves instead of hanging (${thrown?.message ?? ''})`);
    assert.ok(c?.data?.length===3, 'lagging encoder: chunk delivered');
    let second=null;
    try{ second=await withTimeout(enc.push(plane()), 2000, 'lagging push 2'); }catch(e){ thrown=e; }
    assert.ok(!thrown && second, `lagging encoder: subsequent push resolves (${thrown?.message ?? ''})`);
    await enc.close();

});

test('codec error: push() rejects rather than raising an unhandled exception into the void', async () => {
    MODE='error';
    const enc=createTrackEncoder({ kind:'luma', lossless:true, W, H, fps:FPS });
    let thrown=null;
    try{ await withTimeout(enc.push(plane()), 2000, 'failing push'); }catch(e){ thrown=e; }
    assert.ok(thrown && thrown.message==='stub codec exploded', `failing encoder: push rejects with the codec error (${thrown?.message})`);

    let again=null;
    try{ await enc.push(plane()); }catch(e){ again=e; }
    assert.ok(again && again.message==='stub codec exploded', 'failing encoder: later push()es report the same error');

    let onClose=null;
    try{ await enc.close(); }catch(e){ onClose=e; }
    assert.ok(onClose && onClose.message==='stub codec exploded', 'failing encoder: close() reports the error too');

});

test('probe/production config parity', async () => {
    MODE='immediate';
    ...
    assert.ok(JSON.stringify(configs[0])===JSON.stringify(want), `production lossless config is the shared one (${JSON.stringify(configs[0])})`);

    const { probeEncode, _resetProbes }=await import('../src/backend/probe.js');
    _resetProbes();
    assert.ok(await probeEncode({ lossless:false })===true, 'lossy probe runs against the stub');
    ...
    assert.ok(JSON.stringify(probed)===JSON.stringify(prodLossy), `probe asks about the production config (${JSON.stringify(probed)})`);
    ...
    for(const f of ['src/backend/probe.js','src/backend/webcodecs.js']){
      ...
      assert.ok(!/vp09\.|bitrateMode|latencyMode/.test(src), `${f} carries no inline codec settings`);
    }
