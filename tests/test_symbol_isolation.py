...
def check_symbols(lib):
    ...
    if not (exported):
        raise AssertionError("no exported symbols found in %s" % lib)
    non_dc = sorted(n for n in exported if not n.startswith("dc_"))
    if not (not non_dc):
        raise AssertionError("_core exports symbols outside the dc_* ABI: %s" % non_dc[:5])
...
def check_hostile_preload(lib):
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        if not (r.returncode == 0 and "ok" in r.stdout):
            raise AssertionError(
                "round-trip broke with a decoy libvpx preloaded RTLD_GLOBAL — the core is binding "
                "to the foreign libvpx.\nstdout: %s\nstderr: %s" % (r.stdout, r.stderr))
        print("  round-trip bit-exact with a decoy libvpx in the global scope")


class SymbolIsolation(unittest.TestCase):
    def test_all(self):
        failures = []
