...
function throws(fn, re, m){
  try{ fn(); assert.ok(false, `${m}: expected throw`); }
  catch(e){ assert.ok(re.test(String(e)), `${m}: message "${e}"`); }
}
async function rejects(p, re, m){
  try{ await p; assert.ok(false, `${m}: expected rejection`); }
  catch(e){ assert.ok(re.test(String(e)), `${m}: message "${e}"`); }
}
...
assert.ok(u16FromFramePayload({ u16:new Uint16Array(PIX) }, rawSig, PIX).length===PIX, 'exact u16 accepted');
...
assert.ok(u16FromFramePayload({ float:new Float32Array(PIX).fill(1) }, depthSig, PIX).length===PIX,
  'exact float accepted');
// Without the expected count the helper still passes anything through (used where W*H is unknown).
assert.ok(u16FromFramePayload({ u16:new Uint16Array(3) }, rawSig).length===3, 'no length arg -> no check');

test('backend: the memory-safety backstop, exercised directly', async () => {
...

{
  ...
  assert.ok(enc.frameCount===0, 'rejected frames are not counted');
  ...
  assert.ok(enc.frameCount===3, `frameCount after 3 good frames: ${enc.frameCount}`);
  ...
  await dec.close();
  assert.ok(out.length===3, `decoded ${out.length} frames, want 3`);
  for(let i=0;i<out.length;i++) assert.ok(eq(seq[i], out[i]), `frame ${i} bit-exact after rejected frames`);
}
...
// An encoder that reserved an rgb track up front checks the buffer the same way.
{
  ...
  assert.ok(enc.frameCount===0, 'rejected rgb frame is not counted');
}
...
// A wrong-length plane on the *second* signal must not leave the first signal's tracks ahead.
{
  ...
  assert.ok(enc.frameCount===0, 'partially-validated frame is not counted');
  ...
  await dec.close();
  assert.ok(out.length===2, `multi-signal: decoded ${out.length} frames, want 2`);
  assert.ok(out.length===2 && eq(out[0].a.u16, planeAt(0)) && eq(out[0].b.u16, planeAt(1))
