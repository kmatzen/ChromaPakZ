...
def relabel(field, claim):
    ...
    assert len(old) == len(new), f"{field}: {claim} must have as many digits as the real value"
    assert good.count(old) == 1, f"{field}: expected exactly one occurrence, got {good.count(old)}"
...
assert cz.probe(few)["frames"] == 1, "the crafted header should report 1 frame"
for name, call in (("dc_decode_signal", decode_signal_raw), ("dc_decode_rgb", decode_rgb_raw)):
    rc, past = call(few, frames=1, w=W, h=H)
    assert past == 0, f'{name}: wrote {past} units past a 1-frame buffer on "frames":1'
    assert rc == ERR_CAPACITY, f"{name}: expected rc={ERR_CAPACITY} on frame-count overflow, got {rc}"

# a capacity one frame short of the truth is still refused (the off-by-one boundary)
for name, call in (("dc_decode_signal", decode_signal_raw), ("dc_decode_rgb", decode_rgb_raw)):
    rc, past = call(good, frames=N - 1, w=W, h=H)
    assert rc == ERR_CAPACITY and past == 0, f"{name}: N-1 capacity gave rc={rc}, {past} past"
    rc, _ = call(good, frames=N, w=W, h=H)
    assert rc == 0, f"{name}: exactly-N capacity should succeed, got rc={rc}"
...
for field, claim in (("width", 99), ("height", 99)):
    ...
    for name, call in (("dc_decode_signal", decode_signal_raw), ("dc_decode_rgb", decode_rgb_raw)):
        ...
        assert rc == ERR_GEOMETRY, \
            f'{name}: expected rc={ERR_GEOMETRY} for "{field}":{claim} over a {W}x{H} stream, got {rc}'
        assert past == 0, f"{name}: wrote {past} units past the buffer"
...
for field, claim in (("width", 99999), ("height", 99999)):
    ...
    assert rc != 0, f"{field}={claim} should be refused"
...
assert np.array_equal(cz.decode_signal(good, "depth"), depth), "honest signal bit-exact"
back_rgb = cz.decode_rgb(good)
assert back_rgb.shape == (N, H, W, 4), f"honest rgb shape {back_rgb.shape}"
...
assert np.array_equal(cz.decode_signal(short, "depth"), depth[:2]), "2-frame clip bit-exact"
